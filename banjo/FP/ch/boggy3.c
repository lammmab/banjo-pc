...
void func_80390388(Actor *this){
    ...
    if( subaddie_playerIsWithinSphereAndActive(this, 0xFA)
        ...
        if(0.0f == this->unk1C[0] && func_80390334()){
            if(gcdialog_showDialog(ASSET_C1F_DIALOG_BOGGY_NEEDS_SLEEP, 0xb, this->position, N64_NULL, N64_NULL, N64_NULL)){
                this->has_met_before = true;
                this->unk1C[0] = 1.0f;
            }
        }
            ...
            if( !this->has_met_before ){
                if(gcdialog_showDialog(ASSET_C1E_DIALOG_BOGGY_LOOK_FOR_PRESENTS, 0x2b, this->position, N64_NULL, N64_NULL, N64_NULL)){
                    this->has_met_before = true;
                }
            }
        }
